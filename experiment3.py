...
plt.title("SlidingWindow for SGDClassifer in MNIST + C_MNIST Brightness – change in window length")
plt.xlabel("Chunk")
...
for r in results_fisher:
    plt.plot(r["rolling"], label=f"L={r['window']}")
plt.axvline(drift_chunk - 1, linestyle="--", color="black", label="Drift")
plt.title("UnlearningClassifier for SGDClassifer in MNIST + C_MNIST Brightness – change in window length")
plt.xlabel("Chunk")
plt.ylabel("Rolling accuracy")
plt.legend()
plt.tight_layout()
